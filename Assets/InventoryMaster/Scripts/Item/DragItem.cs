...
public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
    ...
    void Start()
    {
        ...
        draggedItemBox = GameObject.FindGameObjectWithTag("DraggingItem").transform;                    // empty object to receive icon
    }
    ...
    public void OnPointerDown(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            oldSlot = transform.parent.gameObject;                                              // store previous location
        }
        if (updateInventoryList != null)
            updateInventoryList();
    }

    public void createDuplication(GameObject Item)                                              // if duplication needed
    {
